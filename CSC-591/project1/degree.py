...
def simple(g):
    ...
    # YOUR CODE HERE
    flipped = g.edges.toPandas().copy()
    flipped.columns = ['dst', 'src']
    flipped = flipped.loc[:, ['src', 'dst']]
    flipped = sqlContext.createDataFrame(flipped)
    ...
    new_edges = flipped.union(g.edges).sort('src')
    new_edges = new_edges.drop_duplicates()
    new_edges = new_edges[new_edges['src'] != new_edges['dst']]
...
def degreedist(g):
    ...
    # YOUR CODE HERE
    node_degree = g.edges.toPandas().groupby('src').dst.nunique()
    node_degree = pandas.DataFrame(node_degree).reset_index()
    node_degree.columns = ['node', 'degree']

    count = node_degree.groupby('degree').node.nunique()
    count = pandas.DataFrame(count).reset_index()
    count.columns = ['degree', 'count']
    count = sqlContext.createDataFrame(count)
...
if len(sys.argv) > 1:
    ...
    print(
            "Original graph has " + str(g.edges.count()) + " directed edges and " + str(
...
# Otherwise, generate some random graphs.
    ...
    for gx in todo:
        ...
        # distrib.toPandas().plot(kind='bar', x='degree', y='count')
        # plt.title(gx)
        # plt.show()
